...
def calculate_completeness(user_id, ob_state):
    score = 0
    if ob_state and ob_state.get("step2_done"): score += 25  # About you
    if ob_state and ob_state.get("step3_done"): score += 25  # Health history
    if ob_state and ob_state.get("step4_done"): score += 25  # Lifestyle & goals
            ...
            dots += f"""
              <div style='display:flex;align-items:center;gap:6px;'>
                <div style='width:27px;height:27px;border-radius:50%;
                            ...
                            font-size:11px;font-weight:600;color:{dot_col};flex-shrink:0;'>
                  {dot_txt}
                </div>
                <span style='font-family:Inter,sans-serif;font-size:11px;
                             color:{lbl_col};font-weight:{lbl_wt};white-space:nowrap;'>
                  {label}
                </span>
              </div>"""
            if i < len(OB_STEPS) - 1:
                dots += f"<div style='flex:1;height:1px;background:{conn_bg};margin:0 4px;'></div>"
        return f"<div style='display:flex;align-items:center;margin-bottom:20px;'>{dots}</div>"

    def ob_title(title, subtitle=""):
        sub_html = f"<div style='font-family:Inter,sans-serif;font-size:13px;color:var(--mid);line-height:1.5;margin-top:4px;'>{subtitle}</div>" if subtitle else ""
        return f"""<div style='margin-bottom:22px;'>
          <div style='font-family:Newsreader,serif;font-size:1.45rem;color:var(--graphite);
                      font-weight:400;line-height:1.2;'>{title}</div>
          {sub_html}
        </div>"""

    _, col_main = st.columns([1, 10])

    with col_main:

        # ── STEP 1 — Your plan ───────────────────────────────────────────────────
        if current_step == 1:
            st.markdown(ob_progress(1), unsafe_allow_html=True)
            st.markdown(ob_title(
                "What are you here for?",
                "Choose your plan mode. This shapes how your coach works with you. You can change it at any time from Profile & Data."
            ), unsafe_allow_html=True)

            PLAN_MODES = [
                ("Restore", "90-day programme", "Hormonal reset, gut repair, thyroid optimisation, fatigue resolution. Structured 13-week phases with checkpoints."),
                ("Optimise", "Ongoing", "You're in good health and want to stay ahead. Performance, longevity, cycle optimisation, cognitive sharpness."),
                ("Targeted", "Condition-specific", "One primary condition driving everything — PCOS, IBS, hypothyroidism, fertility. Laser-focused protocol."),
                ("Maintain", "Ongoing", "Your roadmap is done. Monthly check-ins, protocol maintenance, wearable monitoring."),
            ]

            saved_plan = ob.get("plan_mode", "") if ob else ""
            selected_plan = st.session_state.get("ob_plan_select", saved_plan)

            for key, sub, desc in PLAN_MODES:
                ...
                check = "<span style='color:#111214;font-size:14px;'>✓</span>" if is_sel else ""
                if st.button(f"**{key}** — {sub}\n{desc}", key=f"plan_{key}", use_container_width=True):
                    st.session_state.ob_plan_select = key
                    st.rerun()
                # Visual card overlay
                st.markdown(f"""
                <div style='background:{bg};border:{border};border-radius:10px;
                            padding:14px 16px;margin-bottom:8px;margin-top:-46px;
                            pointer-events:none;position:relative;z-index:-1;'>
                  <div style='display:flex;justify-content:space-between;align-items:flex-start;'>
                    <div>
                      <div style='font-family:Inter,sans-serif;font-size:14px;font-weight:600;
                                  color:var(--graphite);margin-bottom:3px;'>{key}
                        <span style='font-weight:400;color:var(--mid);font-size:12px;'> — {sub}</span>
                      </div>
                      <div style='font-family:Inter,sans-serif;font-size:12.5px;color:var(--mid);
        ...
        # ── STEP 2 — About you ──────────────────────────────────────────────────
            ...
            st.markdown(ob_progress(2), unsafe_allow_html=True)
            st.markdown(ob_title("Tell us about yourself", "This personalises everything. You can edit any of this from Profile & Data at any time."), unsafe_allow_html=True)
        ...
        # ── STEP 3 — Health history ─────────────────────────────────────────────
            ...
            st.markdown(ob_progress(3), unsafe_allow_html=True)
            st.markdown(ob_title("Your health history", "Medical conditions, medications, and supplements. Share what you're comfortable with — more context means better recommendations."), unsafe_allow_html=True)
        ...
        # ── STEP 4 — Lifestyle & goals ──────────────────────────────────────────
            ...
            st.markdown(ob_progress(4), unsafe_allow_html=True)
            st.markdown(ob_title("Lifestyle & goals", "Daily routine, sleep, exercise, and what you want to achieve."), unsafe_allow_html=True)
        ...
        # ── STEP 5 — Data sources ───────────────────────────────────────────────
            ...
            st.markdown(ob_progress(5), unsafe_allow_html=True)
            st.markdown(ob_title("Lab reports & data sources", "Lab reports are the foundation of your treatment roadmap. Upload what you have — even a partial report helps."), unsafe_allow_html=True)
